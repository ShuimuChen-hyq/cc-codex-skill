...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Send a joint research request to Claude and verify it was truly submitted.")
...
def send_raw_tmux(target: str, message: str, tmux_command: str) -> None:
    buf = f"joint-research-{uuid.uuid4().hex[:12]}"
    try:
        proc = subprocess.run(
...
def main() -> int:
    ...
    message = (
        f"Read {doc_path} first. "
        "This is joint research, not a simple review. "
        "Please do one independent research pass, then update the same document with "
        "`## Claude Research`, `## Claude Summary`, and `## Joint Review`. "
        "`Claude Research` should contain evidence and analysis, `Claude Summary` should contain your interim summary, "
        "and `Joint Review` should contain your review of the full document. "
        "If the document does not have a `## Claude Summary` heading yet, add it. "
        "Do not write the final conclusion; Codex owns `## Codex Final Conclusion`. "
        "After writing the file, reply in the pane with JOINT_RESEARCH_WRITTEN."
    )
