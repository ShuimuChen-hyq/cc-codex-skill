...
def main() -> int:
    ...
    message = (
        f"请先读 {doc_path} 。"
        "这是联合调研，不是只做 review。"
        "请你先独立做一轮调研，然后在同一文档中补充 `## Claude Research`、`## Claude Summary` 和 `## Joint Review`。"
        "其中 `Claude Research` 写证据与分析，`Claude Summary` 写你的阶段性汇总，`Joint Review` 写你对当前整份文档的联合审核意见。"
        "如果文档里还没有 `## Claude Summary` 标题，请你补上。"
        "不要写最终结论，最终结论由 Codex 写在 `## Codex Final Conclusion`。"
        "写完后请在 pane 回复一句 JOINT_RESEARCH_WRITTEN。"
    )
